...
export const TemplateM: React.FC<TemplateProps> = ({ data, primaryColor = '#0284c7' }) => {
  ...
  return (
    <div className="bg-white text-slate-800 text-[11px] leading-snug w-full min-h-full font-sans-ui">
      {/* Top Center Avatar matching Column M */}
      <div className="text-center py-5 border-b border-slate-200 space-y-1">
        <div className="flex justify-center mb-2">
          <DefaultAvatar photoUrl={personal.photoUrl} fullName={personal.fullName} sizeClass="w-20 h-20" borderColor="border-sky-500" />
        </div>
      ...
      <div className="grid grid-cols-12">
        {/* Left Column (5 cols): Bright Ocean Teal Sidebar matching Column M */}
        <div className="col-span-5 bg-[#0284c7] text-white p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1.5 text-[10px] text-sky-100">
              ...
              {personal.birthDate && <div className="flex items-center gap-1.5"><Calendar className="w-3 h-3 text-sky-200" /><span>{personal.birthDate}</span></div>}
              {personal.email && <div className="flex items-center gap-1.5"><Mail className="w-3 h-3 text-sky-200" /><span className="truncate">{personal.email}</span></div>}
              {personal.phone && <div className="flex items-center gap-1.5"><Phone className="w-3 h-3 text-sky-200" /><span>{personal.phone}</span></div>}
              {personal.address && <div className="flex items-center gap-1.5"><MapPin className="w-3 h-3 text-sky-200" /><span>{personal.address}</span></div>}
              {personal.github && <div className="flex items-center gap-1.5"><Github className="w-3 h-3 text-sky-200" /><span>{personal.github}</span></div>}
              {personal.linkedin && <div className="flex items-center gap-1.5"><Linkedin className="w-3 h-3 text-sky-200" /><span>{personal.linkedin}</span></div>}
              {personal.website && <div className="flex items-center gap-1.5"><Globe className="w-3 h-3 text-sky-200" /><span>{personal.website}</span></div>}
            </div>
          </div>
              ...
              <div className="space-y-2.5">
                {education.map((edu) => (
                  <div key={edu.id} className="text-[10px] space-y-0.5">
                    ...
                    <div className="text-sky-200 text-[9px]">{edu.endDate}</div>
                    <div className="text-sky-100">{edu.degree}</div>
                  </div>
                ))}
              </div>
            </div>
                ...
                {languages.map((lang, idx) => {
                  ...
                  return (
                    <div key={idx} className="space-y-0.5">
                      <div className="text-[10px] font-medium text-sky-100">{name}</div>
                      <SkillMeter
                        level={level}
                        style={data.skillStyle || 'bars'}
                        isDark={true}
              ...
              <div className="space-y-2 text-[9.5px]">
                {achievements.map((ach) => (
                  <div key={ach.id}>
                    ...
                    <p className="text-sky-100">{ach.description}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Right Column (7 cols): Clean White Content matching Column M */}
                ...
                {experiences.map((exp) => (
                  <div key={exp.id} className="space-y-0.5">
                    <div className="flex justify-between items-baseline font-bold text-[10.5px] text-slate-900">
                      ...
                      <span className="text-[9px] text-sky-600 font-semibold">{exp.startDate} - {exp.current ? 'Present' : exp.endDate}</span>
                    </div>
